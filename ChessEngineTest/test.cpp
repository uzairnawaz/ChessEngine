...
TEST(MoveGeneration, NumStartingMoves) {
    ...
    EXPECT_EQ(c.generateAllLegalMoves(Player::WHITE).size(), 20);
    EXPECT_EQ(c.generateAllLegalMoves(Player::BLACK).size(), 20);
}

TEST(MoveGeneration, RookTest) {
    Chessboard c = Chessboard("k7/1r2B3/7N/7p/8/1q6/8/2K4R b - - 0 1");
    std::vector<Move> wMoves = c.generateAllLegalMoves(Player::WHITE);
    std::vector<Move> bMoves = c.generateAllLegalMoves(Player::BLACK);
...
TEST(MoveGeneration, EnPassantTest) {
    ...
    std::vector<Move> moves = c.generateAllLegalMoves(Player::WHITE);
